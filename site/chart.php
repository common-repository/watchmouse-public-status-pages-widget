...
<div id="<?php echo esc_attr( $theme_name ); ?>">
	<div id="content" class="content-psp" role="main">
		...
		<div id="wm-psp-page" class="chart-page wm-<?php echo esc_attr( $wm_options['theme'] ); ?>"><!--start WM PSP Chart Page-->
			...
			<h1>
				<a href="<?php echo esc_url( $psp_page_url ); ?>" title="<?php printf( esc_attr__('BACK_TO_PSP_PAGE', 'watchmouse-public-status-pages-widget'), esc_attr( $company_name ) ); ?>">
					<?php printf( esc_html__('CURRENT_STATUS', 'watchmouse-public-status-pages-widget' ), esc_html( $client_name ) ); ?>
				</a>
			</h1>
			
			<?php // current availability last update time bubble ?>
			<h2><?php esc_html_e('CURR_PERF_AVAIL_STAT', 'watchmouse-public-status-pages-widget' ); ?></h2>
			<div id="wm-psp-curr-status" class="wm-hidden">
				<div id="psp_last_update" class="wm-hidden">&nbsp;</div>
			</div>
			
			<?php // charts will be printed there ?>
			<div id="wm-chart-widget-<?php echo esc_attr( $post->ID ); ?>" class="wm-chart-widget"><!--start printing of charts-->
				<?php
